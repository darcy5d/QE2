#!/usr/bin/env python3
"""
Train Model 3: Absolute Speed Regression
XGBoost regressor predicting speed in furlongs/second
"""
...
class SpeedModelTrainer:
    """Train absolute speed regression model"""
    ...
    def load_data(self, test_size: float = 0.2) -> Tuple:
        """
        Load features and compute speed target from time and distance
        ...
        # Load features with time and distance to compute speed
        ...
        # Speed statistics
        logger.info(f"\nSpeed target statistics:")
        logger.info(f"  Mean: {df['speed'].mean():.4f} f/s")
        logger.info(f"  Median: {df['speed'].median():.4f} f/s")
        logger.info(f"  Std: {df['speed'].std():.4f} f/s")
        logger.info(f"  Range: {df['speed'].min():.4f} to {df['speed'].max():.4f} f/s")
        ...
        # Extract features and speed targets
        X_train = train_df[self.FEATURE_COLS].copy()
        X_test = test_df[self.FEATURE_COLS].copy()
        y_train = train_df['speed'].copy()
        y_test = test_df['speed'].copy()
    ...
    def train_xgboost_regressor(self, X_train: pd.DataFrame, y_train: pd.Series,
        ...
        Target: Speed (furlongs/second) - continuous value
        Higher = faster (better)
        """
        logger.info("\n" + "="*60)
        logger.info("TRAINING XGBOOST SPEED REGRESSION MODEL")
        logger.info("="*60)
    ...
    def evaluate(self, model: xgb.Booster, X_test: pd.DataFrame, 
        ...
        """
        Evaluate speed regression model
        
        Metrics:
        - Regression: RMSE, MAE, R²
        - Ranking: Convert speed to ranks and check top pick accuracy
        """
        logger.info("\n" + "="*60)
        ...
        # === RANKING QUALITY ===
        logger.info("\n🏆 RANKING QUALITY (from speed predictions)")
        
        test_df_copy = test_df.copy()
        test_df_copy['predicted_speed'] = y_pred
        test_df_copy['actual_speed'] = y_test
        
        # Rank by predicted speed (highest = winner)
        test_df_copy['pred_rank'] = test_df_copy.groupby('race_id')['predicted_speed'].rank(
    ...
    def save_model(self, model: xgb.Booster, metrics: Dict, output_dir: Path):
        ...
        # Save model
        model_path = output_dir / 'xgboost_flat_speed_abs.json'
        ...
        importance_path = output_dir / 'feature_importance_speed_abs.csv'
        metrics['feature_importance'].to_csv(importance_path, index=False)
        logger.info(f"✓ Feature importance saved to {importance_path}")
        
        # Save metrics
        metrics_path = output_dir / 'speed_abs_model_metrics.json'
        metrics_to_save = {
            'rmse': metrics['rmse'],
            'mae': metrics['mae'],
            'r2': metrics['r2'],
            ...
            'mrr': metrics['mrr'],
            'model_type': 'speed_absolute_regression',
            'target': 'speed_furlongs_per_second',
...
def main():
    """Run absolute speed model training"""
    # Paths
    db_path = Path(__file__).parent.parent / 'racing_pro.db'
    output_dir = Path(__file__).parent / 'models'
    
    # Initialize trainer
    trainer = SpeedModelTrainer(db_path, race_type='Flat')
    ...
    try:
        ...
        logger.info("\n" + "="*60)
        logger.info("✅ MODEL 3 (ABSOLUTE SPEED REGRESSION) TRAINING COMPLETE")
        logger.info("="*60)
        logger.info(f"RMSE: {metrics['rmse']:.6f} f/s")
        logger.info(f"Top Pick Accuracy: {metrics['top_pick_accuracy']:.1f}%")
        logger.info(f"Model saved to: {output_dir / 'xgboost_flat_speed_abs.json'}")
